...
router.put('/cart/:id', checkAuthentication, async (req, res) => {
  try {
    ...
    const user = req.user;
    const bookInCart = user.carts.find(cartItem => cartItem.book.equals(book._id));

    if (bookInCart) {
      console.error('Book is already in the cart');
      return res.redirect('back');
    }
    ...
    // Save the updated user to the database using promises
    await user.save();
...
router.get('/dashboard', checkAuthentication, async (req, res) => {
  ...
  if (req.user.role !== 'admin') {
    try {
      const user = await User.findById(req.user.id).populate('carts.book').exec();
      // Handle the result
      res.render('users/dashboard', { user: user });
    } catch (err) {
      console.error(err);
...
router.post('/checkout', checkAuthentication, async (req, res) => {
  try {
    ...
    // Retrieve the updated user with populated carts
    const user = await User.findById(oldUser.id).populate('carts.book');
